...
final class Uri implements UriInterface
{
    ...
    public function getPath(): string
    {
        ...
        // Reduce multiple leading slashes to one to avoid XSS/URL-poisoning when
        // the path is used without the authority (CVE-2015-3257).
    ...
    public function __toString(): string
    {
        ...
        if ($path !== '') {
            if ($path[0] !== '/') {
                if ($authority !== '') {
                    // A rootless path with an authority must be prefixed with "/".
                    $path = '/' . $path;
                }
            } elseif (isset($path[1]) && $path[1] === '/' && $authority === '') {
                // A path starting with "//" without an authority must be reduced.
                $path = '/' . ltrim($path, '/');
            }
