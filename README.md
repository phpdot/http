...
`HttpConfig` is tagged `#[Config('http')]`, so when used inside the phpdot framework (with `phpdot/package` + `phpdot/config`), it's hydrated automatically from `config/http.php`:

```php
// config/http.php — auto-generated by phpdot/package on first composer install
use PHPdot\Http\Request;
...
Edit the file to configure your deployment. **Do this for any deployment behind a proxy / load balancer / CDN — without it, `Request::ip()`, `Request::isSecure()`, `Request::host()`, etc. silently return wrong values.**
...
Request::initFromConfig($container->get(HttpConfig::class));
```

`Request::initFromConfig()` is a thin convenience over `Request::setTrustedProxies()` that reads the DTO and applies it. The static `setTrustedProxies()` API is unchanged — apps without `HttpConfig` continue to call it directly.
...
```bash
composer test        # PHPUnit (249 tests)
